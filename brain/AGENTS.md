...
### 19. Sun Biz Agent (Funding Operations — Client-Tenanted, added 2026-05-11)
...
- **GitHub:** CC90210/SunBiz-Marketing
- **Purpose:** CC's first **client-tenanted backend operations agent**. Runs day-to-day funding operations for the Sun-tenant (Sun Biz Funding LLC) — lead sourcing, multi-provider SMS + email outreach, application/offer/funded-deal lifecycle, renewal triggers, commission tracking, lender CRM.
...
- **Dashboard tenancy:** `agent-dashboard-cc90210.vercel.app` serves the Sun-tenant funding-ops sidebar when the authed profile resolves to `tenant_slug = "sun"`; CC's empire nav is unchanged for other tenants.
- **Relationship to Bravo:** Bravo is CC's architect — owns shared substrate (state_manager, event_bus, migrations, dashboard chrome). Sun Biz Agent is a tenant operator — owns Sun's day-to-day funding ops. Sun Biz READs from Business-Empire-Agent (substrate utilities) and writes through it (events, state DB, Supabase tenant_id-scoped tables). Bravo MAY read Sun Biz pulse/state but does NOT mutate Sun Biz repo files unless explicitly invoked for substrate work.

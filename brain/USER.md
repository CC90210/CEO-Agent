---
tags: [user, profile]
---

# USER — Conaugh McKenna's Complete Profile

> Bravo reads this to understand Conaugh McKenna's world. Update when CC shares new info.

## Identity

- **Full Name:** Conaugh McKenna (CC)
- **Personal Brand:** Kona Makana
- **Age:** 22
- **Location:** Collingwood, Ontario, Canada
- **International Experience:** Lived in Osaka, Dublin, Oslo, Spain, Portugal
- **Education:** Dropped out of university (Bishop's University)
- **Weekend Job:** Nicky's Donuts
- **DJ:** Serato DDJ-202, venues like Taps and Tunes
- **Fitness:** Regular gym routines, peptide research

## Primary Objective

**"$5,000 USD Net MRR by May 15, 2026."**
> Previous goal ($1,000 USD Net MRR by March 31, 2026) — **ACHIEVED** at $2,691 USD (+169%).

Current net MRR is ~$3,322 USD ($180 Stripe + $191 base + $2,500 primary retainer flat + $451 primary retainer 15% rev share on $3,007 Skool community MRR). Skool community: 158 members, 63% engagement, 100% retention, 159 signups last 30d, 5.5% conversion. Rev share grows as community grows. Gap: ~$1,678 USD/mo (~2 new clients). Every action drives toward diversification and pipeline growth.

## Personal Mission & Values

- **High-Leverage Partner:** Build real relationships with clients, showcase undeniable value, and maintain absolute leverage.
- **The Educator:** Teach people about AI, eliminate fear, position AI as the ultimate "Swiss Army Knife" for business growth.
- **The Engine:** Achieve $1k MRR through relentless outreach, closing, and value-first execution.
- **Philosophy:** "Only good things from now on."
- **Passions:** Music, DJing, audio culture, rhythm.

## Operational Weaknesses (Bravo's Job to Fix)

| Weakness | How Bravo Compensates |
|----------|----------------------|
| Follow-through | I am the finisher. I close every loop and deploy every pipeline without being asked. |
| Consistency | I enforce the daily outreach and content routines that lead to $1k MRR. |
| Conviction | I provide the all-knowing, all-powerful business expertise that converts prospects. |
| Organization | I maintain the structure required to scale OASIS while you sleep. |

## Business Portfolio

| Brand | Type | Focus |
|-------|------|-------|
| **OASIS AI Solutions** | AI Automation Agency | Retainers for local service businesses (HVAC, Wellness). |
| **PropFlow** | Real Estate SaaS | Real estate CRM/Automation. **50-50 with Adon.** |
| **Nostalgic Requests** | Music/DJ SaaS | Dark-theme request system for live events. |
| **Kona Makana** | Personal Brand | Content, consulting, and building in public. |
| **DJ Services** | Entertainment | Live sets, gig booking, and audio culture branding. |
| **Consulting** | Business Strategy | High-level leverage and automation strategy for entrepreneurs. |

**Partner:** Adon — 50-50 on PropFlow only. Responsible for networking, connections, marketing. CC owns 100% of OASIS AI. Technically 3-4 months behind CC but valuable for network access.

## CC's Role (Non-Delegable)

| CC Does | Bravo Handles |
|---------|---------------|
| Content creation (personal brand, video, posts) | Scheduling, distribution, repurposing, analytics |
| Sales calls, face-to-face meetings | Lead research, follow-up drafts, CRM updates |
| Client relationship management | Deliverable tracking, invoice reminders, health scoring |
| Strategic decisions (pricing, partnerships) | Data gathering, option analysis, recommendations |
| Coaching/teaching sessions | Session prep, materials, scheduling, follow-ups |

## Critical Links (Never Guess These)
- **Booking Link:** `https://calendar.app.google/tpfvJYBGircnGu8G8` (DO NOT use Calendly)
- **Website:** `https://oasisai.work`
- **Primary Email:** `conaugh@oasisai.work`

## Content Voice

- Introspective, raw, honest, never preachy.
- High-leverage, authority-driven, personable.
- Write like talking to a friend at 2am.
- Close with "Only good things from now on." when appropriate.

## Mindset

**"Done with money problems."**
Every action is fueled by the drive to build real wealth and a business empire from every dollar (USD) earned.

*Last synced: 2026-03-26*

## Obsidian Links
- [[brain/SOUL]] | [[brain/STATE]] | [[brain/CEO_OPERATING_SYSTEM]]
- [[memory/ACTIVE_TASKS]] | [[memory/content-strategy]] | [[brain/DASHBOARD]]

...
| Hostinger GPU box (MavenGPU) — video generation | `gpu/oasisgpu.py {set-host\|show\|smoke\|bootstrap\|stack\|shell\|run\|push\|tunnel\|pull}` | **LIVE 2026-09-17**, first clip rendered. RTX PRO 6000 / 94GB VRAM / 64GB RAM / **2 CPU (cgroup quota — `nproc` lies and says 128)** / ~89GB disk. GPU is **not** in the Hostinger API; IP+port come off hPanel → Dev tools → GPU → Manage → Overview (port is per-instance, e.g. 31310), saved via `set-host <ip> --user ubuntu --port <p>`. Key-only auth (`~/.ssh/oasisgpu_ed25519`; public half registered on the Hostinger account as `oasisgpu-automation`). **Never expose ComfyUI — it has no auth. Use `tunnel 8188`.** `push <file>` takes no remote path (Git Bash rewrites `/tmp/...` into a Windows path). Install: `bootstrap` → `stack` (both idempotent, `--plan` dry-runs). |
| Generate a video clip on the GPU | `gpu/oasisgpu.py push scripts/gpu/wan_gen.py` then `gpu/oasisgpu.py run '/opt/oasis/venv/bin/python /tmp/wan_gen.py {t2v\|i2v\|s2v} --prompt "..." --name <n>'` (i2v also needs `--image <path on box>`) | **`i2v` is the quality tier** — Wan 2.2 I2V-A14B two-expert MoE from an art-directed first frame. `t2v` (5B) is faster but reads dated; CC rejected a T2V sample on 2026-09-17. Apache-2.0 → client-safe, outputs unencumbered; lightx2v 4-step LoRAs also verified Apache-2.0. Measured: 832x480/49f/20steps = **82s**. Retrieve with `pull outputs`. Graphs pinned by `scripts/tests/test_wan_gen_graph.py` (9 tests — the MoE handoff flags degrade output *silently* when wrong). **Weights must be `Comfy-Org/Wan_2.2_ComfyUI_Repackaged` — official `Wan-AI/*` repos ship sharded diffusers that UNETLoader cannot open** (cost 32GB once). **S2V audio encoder must live in `ComfyUI/models/audio_encoders/`** — `extra_model_paths.yaml` maps no such category and the loader shows an empty list with no error. **One GPU, one queue: claim the box before submitting — Bravo and Maven share it.** Maven's handover: `~/CMO-Agent/HANDOVER_MAVENGPU_VIDEO_GENERATION.md`. |
| Hostinger VPS fleet / account | `integrations/hostinger_tool.py {whoami\|vps list\|vps get\|ssh-keys\|add-key\|billing\|gpu probe\|raw}` (**BUILT 2026-09-16**) | `add-key` is the ONLY mutation (POSTs a public key to the account; refuses anything containing `PRIVATE KEY`). Account keys become selectable in the SSH-key field on future GPU/VPS deploys — the fix for "instance came up with `SSH key: -`". Token verified live 2026-09-16 (`.env.agents` `HOSTINGER_API_TOKEN`; base `https://developers.hostinger.com`; **browser User-Agent required** — Cloudflare 1010s plain clients; GPU product NOT in API, hPanel-only). Full context + build spec: `docs/handovers/2026-09-16_hostinger_gpu_llm_handover.md` |

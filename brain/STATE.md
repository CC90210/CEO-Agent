...
| **OASIS AI · Agent Command Center** | ✅ LIVE (2026-04-30, SEPARATED) | https://agent-dashboard-cc90210.vercel.app · Next.js 15 + React 19 + Tailwind + recharts. Auth: Supabase email+password + Google OAuth (login/signup/forgot/reset/signout). Multi-tenant via tenants + RLS (migrations 017+018+019). 11 pages: Today (weekday/weekend templates), Pipeline, Reasoning, Playbook (script + 4 prospect tracks + objections + deals + drills), Agents, Analytics, Integrations, Settings. n8n inbound webhook: POST /api/inbound/n8n. Env: BRAVO_SUPABASE_URL, BRAVO_SUPABASE_SERVICE_ROLE_KEY, NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY, OPERATOR_EMAIL=conaugh@oasisai.work, CRON_SECRET. |

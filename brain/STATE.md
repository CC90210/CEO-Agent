...
| **OASIS AI · Agent Command Center** | ✅ V3 LIVE (2026-04-30 PM) | https://agent-dashboard-cc90210.vercel.app · Next.js 15 + React 19 + Tailwind + recharts. **Brand: OASIS BLUE (#3b82f6)** flipped from gold. Auth: Supabase email+password + Google OAuth (enabled via API). Multi-tenant via tenants + RLS (migrations 017+018+019). 11 pages, all interactive. **Settings page is real**: ProfileEditor (full_name/brand/MRR/manifesto/agents-enabled toggles) + PlanTemplateEditor (weekday + weekend, add/remove/reorder schedule blocks, 'Load defaults' button, auto-materializes today's plan on save) + ChangePasswordForm + IntegrationDot grid. **Today page**: TodayBlockToggle marks blocks completed (optimistic UI, persists). API routes (all session-cookie-gated): GET/PATCH /api/profile, GET/PUT/DELETE /api/plan-templates, PATCH /api/daily-plan, POST /api/daily-plan/materialize. Bearer-auth API: POST /api/auth/provision-cli (setup wizard signup, end-to-end smoke-tested), POST /api/inbound/n8n (n8n webhook). Middleware: 401 JSON for /api/*, 307 to /login for pages. CLI signup secret in Vercel: CLI_SIGNUP_SECRET (also needed in client install env). Shared utility: lib/api-helpers.ts (bad + profileForUser). Webhook contract for clients: docs/COMMAND_CENTER_WEBHOOK_API.md. NO realtime websocket yet (server-rendered fast-polling). NO Stripe / cross-Supabase bridge — Command Center and OASIS marketing are deliberately separate products. |

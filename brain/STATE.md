...
| **OASIS AI · Agent Command Center** | ⚠️ DOMAIN IN LIMBO (2026-04-30 PM) | Direct URL works: https://agent-dashboard-cc90210.vercel.app · oasisai.work CURRENTLY 404 due to botched domain transfer attempt — TXT verification pending CC's manual click in Vercel UI (Settings -> Domains -> Verify on oasis-ai-platform project). Next.js 15 + React 19 + Tailwind + recharts. Auth: Supabase email+password + Google OAuth ENABLED via API 2026-04-30 PM (login/signup/forgot/reset/signout). Multi-tenant via tenants + RLS (migrations 017+018+019, all applied + Google-OAuth-flow verified live via authorize endpoint). 11 pages. Bridge to oasis-ai-platform: POST /api/auth/provision-from-stripe (Bearer auth, idempotent, end-to-end smoke-tested). n8n inbound: POST /api/inbound/n8n + email_engine.py check-inbox via scheduler (KEEPING n8n + Hostinger setup, NO VPS per CC 2026-04-30 PM — VPS artifacts deleted). Env: BRAVO_SUPABASE_URL, BRAVO_SUPABASE_SERVICE_ROLE_KEY, NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY, OPERATOR_EMAIL=conaugh@oasisai.work, BRIDGE_SECRET, PUBLIC_APP_URL, CRON_SECRET. |

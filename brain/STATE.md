...
| **OASIS AI · Agent Command Center** | ✅ V1.0 REBUILT (2026-04-30) | https://agent-dashboard-cc90210.vercel.app — Next.js 15 + React 19 + Tailwind, sidebar nav, 9 pages (Today, Pipeline, Reasoning, Playbook + 4 sub-pages, Agents, Analytics, Integrations, Settings), recharts, framer-motion, lucide-react. Multi-tenant: profile-driven brand/MRR/agents/manifesto/script via user_profiles (migration 017). n8n inbound bridge live: POST /api/inbound/n8n + sha256 shared-secret. Env vars: BRAVO_SUPABASE_URL + BRAVO_SUPABASE_SERVICE_ROLE_KEY + OPERATOR_EMAIL. Setup wizard hook: `python install/bootstrap.py --create-command-center-account`. n8n bridge fix doc: docs/N8N_INBOUND_WEBHOOK.md. |

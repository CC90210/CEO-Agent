...
| omniroute-model-router | Claude usage-limit failover router (fallback leg of Claude Spillover) | [OmniRoute](https://github.com/diegosouzapw/OmniRoute) | MIT, self-hosted | lane key (scoped OmniRoute API key, DPAPI blob) | adopted | fallback leg only, behind our own base-URL-only spillover proxy (ADR-0018); never holds a Claude credential. Pinned to unreleased `release/v3.8.51` sha 152d9510 (Next.js 16.3.3): npm 3.8.50 bundles Next.js 16.3.1 with a Windows unauthenticated RCE (CVE-2026-75604) and binds every interface with no key, so `install` refuses it. Built BACKEND-ONLY with Turbopack (`OMNIROUTE_BUILD_BACKEND_ONLY=1`, `OMNIROUTE_BUILD_MEMORY_MB=4096`, `NEXT_TELEMETRY_DISABLED=1`, `npm ci --ignore-scripts`). At this pin: full builds pull Node-only modules into the dashboard's browser bundle under both bundlers; the full Turbopack build also exhausts the 15 GB box's memory; and `minimal` fails under webpack (root-relative aliases, `Can't resolve './src/lib/cloudSync.stub.ts'`). With no dashboard UI, Codex connects via the device flow and keys via the API. The supervisor supplies OmniRoute's four signing secrets from DPAPI so they stay stable across restarts (CONTRACT §16). The privileged modules (MITM cert installer, Zed keychain import, Cloud Sync, 9router installer) stay compiled in, behind LOCAL_ONLY + required login + loopback bind + doctor checks | 2026-09-12 |
...
| cerebras-free-inference | second fallback tier, after GPT | [Cerebras](https://www.cerebras.ai/privacy-policy) | one-time $5 signup credit (OmniRoute catalog, 2026-09-03), not a recurring free tier | apiKey (`CEREBRAS_API_KEY` in .env.agents, copied into OmniRoute's encrypted DB by `connect-key --from-env-agents`) | candidate | connected 2026-09-13, but every call returns HTTP 402 "credits exhausted". Joins bravo-fallback only after the tool-use smoke gate (`omniroute_tool.py smoke --tools --stream`) passes, which needs account credit first. Terms fit: "We do not retain inputs and outputs associated with our training, inference and chatbot Services" (privacy policy, effective 2024-08-27) | 2026-09-13 |
| zai-api | fallback models for the bravo-fallback combo | [Z.AI](https://docs.z.ai/legal-agreement/privacy-policy) (international api.z.ai endpoint only) | free API key | apiKey | rejected | CC dropped Z.AI from the chain on 2026-09-13, and `connect-key` no longer offers it. The PRC endpoint is the separate rejected glm-cn row | 2026-09-13 |
| cloudflare-workers-ai | third fallback tier, after GPT and Cerebras | [Cloudflare Workers AI](https://developers.cloudflare.com/workers-ai/platform/data-usage/) | small free daily allocation (~10K neurons, OmniRoute registry) | apiKey (`CLOUDFLARE_API_TOKEN`) plus the account id (`connect-key cloudflare --account-id`) | candidate | terms fit: no customer content is used to train models or improve services without consent (data-usage page, 2026-04-21). Connected 2026-09-13, but the existing token gets HTTP 401 from Workers AI on both accounts; it needs a token with Workers AI permission before the smoke gate can run | 2026-09-13 |
...
| glm-cn | GLM models via the mainland-China endpoint | [Zhipu BigModel (CN)](https://open.bigmodel.cn) | n/a | apiKey | rejected | PRC endpoint (open.bigmodel.cn): session and client content must not route there; `glm-cn/` is a forbidden prefix. GLM is out of the chain entirely — CC dropped the international Z.AI endpoint on 2026-09-13 too (see the zai-api row), so there is no sanctioned route to GLM models | 2026-09-13 |
...
- **2026-09-12 (Claude Spillover, ADR-0018):** Added 12 Section 9 rows for the OmniRoute fallback leg — 2 adopted (omniroute-model-router, codex-chatgpt-oauth-via-omniroute), 2 candidates (cerebras-free-inference, cloudflare-workers-ai), 1 policy (opencode-free), 7 rejected (omniroute-claude-provider, gemini-cli-oauth-proxy, antigravity-oauth, kiro, groq-tool-use, glm-cn, and zai-api — CC dropped Z.AI from the chain on 2026-09-13). **Re-check the OmniRoute pin when npm 3.8.51+ ships** — swap from sha 152d9510 to the release once it bundles Next.js ≥ 16.3.3, and re-evaluate the `minimal` build profile then. **Re-check provider terms quarterly** — Cerebras no-retention, Cloudflare Workers AI data usage, and the ChatGPT training toggle still off.

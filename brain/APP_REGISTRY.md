...
| **CC Funnel** | cc-funnel | `C:\Users\User\APPS\cc-funnel` | CC90210/cc-funnel | phctllmtsogkovoilwos (Bravo) | Next.js 14, TypeScript, Tailwind, Supabase | **RETIRED 2026-06-18** → native funnels at oasisai.work (oasis-command-center). **TWO live forms, same tenant `oasis-ai-cc`:** `/f/oasis-ai-cc/start` = personal brand (AI / DJ / brand, 3 steps) · `/f/oasis-ai-cc/ai-audit` = B2B AI qualification (4 steps, scored 0-100 → `leads.score`+`status`, writes a `lead_interactions` row, added 2026-07-29). Each has its OWN seed script — `scripts/seed-oasis-funnel.ts` and `scripts/seed-ai-audit-funnel.ts` — and editing the seed file changes nothing until it is re-run. **Publishing a CTA at either funnel: [[docs/onboarding/MAVEN_FUNNEL_AND_CTA_HANDOVER]]** (which funnel for which content, what the pipeline promises, and the fact that UTM params are NOT captured). |

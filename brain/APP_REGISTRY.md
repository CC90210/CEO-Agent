...
| **Arthrisil** | arthrisil, arthrisil.com | `C:\Users\User\APPS\arthrisil-website` | CC90210/arthrisil-website | — | Next.js, React, Tailwind | **NOT live at arthrisil.com** (verified 2026-09-08: the domain resolves to 160.153.0.56 / GoDaddy and still serves the old WordPress site). The rebuild deploys on push to `main` → Vercel (`arthrisil-website.vercel.app`) + Cloudflare Workers. PayPal + YouTube + $29.95 offer; lead capture → Resend/Supabase, but **zero env vars are set on either target**, so /api/lead fail-softs: the form reports success and discards the lead. Cutover to arthrisil.com = a GoDaddy DNS change, not done (arthrisil.com is NOT a Cloudflare zone). **Vercel pushes land as PREVIEW** — production needs `npx vercel promote <url>`. **The CF workflow's deploy step SKIPS** (no `CLOUDFLARE_API_TOKEN` repo secret) while the run still reports success — deploy with `wrangler_tool.py deploy --app arthrisil-website`. Worker: `arthrisil-website.oasisaisolutions.workers.dev`; OpenNext sets `s-maxage=31536000`, so verify with `?cb=$(date +%s)` or you will read year-old cache. |
...
| **Listing Studio (Mandy Management)** | listing studio, mandy, mandy management, real-estate-marketing-suite, rems, new haven apartments | `C:\Users\User\APPS\real-estate-marketing-suite` (local clone) + VPS `/srv/listing-studio` | CC90210/real-estate-marketing-suite (private) | — (dedicated Turso DB `real-estate-marketing-suite`) | Next.js, Turso/libSQL, PM2 workers (`rems-render`, `rems-publish`), Zernio/Late API, Twilio, Telegram bot @Sexyrapeezra_bot | **Vercel — LIVE at https://real-estate-marketing-suite.vercel.app** (auto-deploys on push to `main`). Listing-video rendering + rental leads CRM. VPS system messages: `docs/LISTING_STUDIO_VPS_AGENT_SYSTEM_MESSAGE.md` (2026-08-19 missions) + `docs/LISTING_STUDIO_OUTREACH_CRM_SYSTEM_MESSAGE.md` (2026-08-24 outreach CRM). |
| **Persona Studio** | persona studio, persona-studio, stephanie, stephanie geraets, avatar studio, heygen pipeline, personal brand studio | `C:\Users\User\APPS\persona-studio` | CC90210/persona-studio (private) | Turso (planned; one DB per deployment, tenant-ready schema) | Python (ffmpeg + Pillow post-production), Next.js (planned web/worker API), HeyGen API, Zernio, Telegram | Demo kit renders on CC's box now; pipeline runs on the OASIS Hostinger VPS (PM2 `ps-*`) after deposit. OASIS's personal-brand AI-avatar content product; client #1 Stephanie Geraets (Bosley Real Estate Ltd., Brokerage). Plan: `docs/PLAN.md`. No Claude subscription in the product runtime (Gemini CLI adapter). |

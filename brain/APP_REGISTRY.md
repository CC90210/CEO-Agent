...
| **Arthrisil** | arthrisil, arthrisil.com | `C:\Users\User\APPS\arthrisil-website` | CC90210/arthrisil-website | — | Next.js, React, Tailwind | **LIVE at arthrisil.com on Cloudflare Workers** (re-verified 2026-09-17: `HTTP 200`, `x-opennext: 1`, `Server: cloudflare`, no `x-vercel-id` — the 2026-09-08 "NOT live / GoDaddy WordPress" note was superseded by the DNS cutover). **Vercel serves nothing**: the project is still linked (`prj_AyrA4nLf6nL7FmHClg59V6zrFbwx`) with **zero env vars** — an empty live-looking shell; if anything ever routes there, `lib/db/client.ts` throws on every request. Commerce is **Stripe Checkout** as of 2026-09-17 (PayPal no-code button removed): `/api/checkout` builds the session server-side, `/api/stripe/webhook` records to `arthrisil_orders`. Three offers: one-time $34.95 + $5.99, **buy-2-get-1-free** (3 bottles $69.90 + $9.99 handling, billed as ONE line item), and monthly $33.20 — US-only. Money is defined once in cents in `lib/pricing.ts`; `lib/site.ts` display strings, the Stripe line items and the price card all derive from it, and `lib/stripe.test.ts` asserts the page total equals the Stripe total at every plan/quantity. **Monthly is pre-selected, never auto-applied** — auto-enrolling a one-time buyer is a negative option without consent (FTC rule + Stripe ToS) and arrives as chargebacks; a test pins it. Cancel path is `/manage` → Stripe customer portal (hosted login page emails the customer, so it works without our email; cancel at period end). Webhook records the full lifecycle into `arthrisil_subscriptions` + `arthrisil_subscription_invoices`. **Tools (both read the app dir from `config/cloudflare/apps.json`):** `arthrisil_stripe_setup.py {plan|apply|webhook|portal}` — `plan` parses `lib/pricing.ts` and exits non-zero on drift; `arthrisil_db.py {migrate|tables|orders|subs|churn}` — `migrate` honours the `schema_migrations` ledger, `churn` is the retention worklist. Keys are `Trytan_Health_Secret_key` + `STRIPE_WEBHOOK_SECRET_TRYTAN` (**Trytan Health's own Stripe account `acct_1UFyxVDfiBJj2XdS` — never OASIS's `STRIPE_SECRET_KEY`**; list key NAMES rather than guessing them). Lead capture reaches Turso, but `RESEND_API_KEY` is unset so **no auto-reply or internal notification is ever sent** while /api/lead still reports success. **The CF workflow's deploy step SKIPS** (no `CLOUDFLARE_API_TOKEN` repo secret) while the run still reports success — deploy with `wrangler_tool.py deploy --app arthrisil-website`. Worker: `arthrisil-website.oasisaisolutions.workers.dev`; OpenNext sets `s-maxage=31536000`, so verify with `?cb=$(date +%s)` or you will read year-old cache. |
| **oasis-store (subscription storefront)** | oasis store, oasis-store, dropship store, subscription store, the store, ecom store | `C:\Users\User\APPS\oasis-store` | CC90210/oasis-store (private) | — (own Turso DB `oasis-store`) | Next.js 16, React 19, Tailwind v4, OpenNext on Cloudflare Workers, Stripe (own dedicated account), R2, KV; Node ops daemon (PM2 `oasis-store-ops`) | **PREVIEW LIVE** at oasis-store.oasisaisolutions.workers.dev (2026-09-18). Brand-agnostic codename — brand name is a settings row; domain + Stripe account bound after product research. Product = data: admin creates a product (starter template), uploads media, syncs Stripe Prices, flips unlisted/live — no deploy. Ops: `oasis_store_db.py`, `oasis_store_stripe_setup.py`, `oasis_store_cf_bootstrap.py`. Never OASIS/PropFlow/Nostalgic Stripe keys. |
...
**Arthrisil marketing suite (2026-09-15):** `/admin` is live inside the existing Cloudflare Worker with employee auth, dedicated Turso database `arthrisil-studio`, R2 bucket `arthrisil-media`, media upload, vertical product rendering, mandatory review, and fail-closed Zernio publishing. VPS processes are `arth-render` and `arth-publish`; setup contract: `docs/ARTHRISIL_VPS_AGENT_SYSTEM_MESSAGE.md`. Live publishing remains disabled until Arthrisil's Zernio profile IDs are verified.

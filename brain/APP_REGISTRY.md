...
| **Arthrisil** | arthrisil, arthrisil.com | `C:\Users\User\APPS\arthrisil-website` | CC90210/arthrisil-website | — | Next.js, React, Tailwind | **NOT live at arthrisil.com** (verified 2026-09-08: the domain resolves to 160.153.0.56 / GoDaddy and still serves the old WordPress site). The rebuild deploys on push to `main` → Vercel (`arthrisil-website.vercel.app`) + Cloudflare Workers. PayPal + YouTube + $29.95 offer; lead capture → Resend/Supabase, but **zero env vars are set on either target**, so /api/lead fail-softs: the form reports success and discards the lead. Cutover to arthrisil.com = a GoDaddy DNS change, not done. |

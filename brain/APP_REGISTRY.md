...
| **nomad-store (subscription storefront)** | nomad, nomad-store, dropship store, subscription store, the store, ecom store | `C:\Users\User\APPS\nomad-store` | CC90210/nomad-store (private) | — (own Turso DB `nomad-store`) | Next.js 16, React 19, Tailwind v4, OpenNext on Cloudflare Workers, Stripe (own dedicated account), R2, KV; Node ops daemon (PM2 `nomad-ops`) | **PREVIEW LIVE** at nomad-store.oasisaisolutions.workers.dev (2026-09-18). Brand-agnostic codename — brand name is a settings row; domain + Stripe account bound after product research. Product = data: admin creates a product (starter template), uploads media, syncs Stripe Prices, flips unlisted/live — no deploy. Ops: `nomad_db.py`, `nomad_stripe_setup.py`, `nomad_cf_bootstrap.py`. Never OASIS/PropFlow/Nostalgic Stripe keys. |

...
> Complete inventory of what Bravo can do. Last reviewed: 2026-09-23.
>
...
## GPU Compute — MavenGPU (self-hosted video generation, LIVE 2026-09-17)

> Replaces **Higgsfield** (video gen) and, once S2V is exercised, **HeyGen** (avatar). Owned hardware, no per-clip credits. Content ownership is Maven's; Bravo owns the box.

| Item | Detail |
|---|---|
| Hardware | Hostinger GPU `MavenGPU` — RTX PRO 6000 Blackwell, **94 GB VRAM**, 64 GB RAM, **2 CPU** (cgroup quota; `nproc` reports the host's 128), ~89 GB disk |
| Reach | `ubuntu@<ip> -p <per-instance port>`, **key-only**; driver `scripts/gpu/oasisgpu.py` |
| Stack | ComfyUI (GPL-3.0) + **Wan 2.2 (Apache-2.0 — outputs unencumbered, client-safe)** + GGUF loader + WanVideoWrapper + VideoHelperSuite + ffmpeg + faster-whisper |
| Models | `wan22-ti2v-5b-comfy` (T2V+I2V, **proven**) · `i2v-a14b-gguf` · `s2v-14b-gguf` (avatar — **downloaded, not yet exercised**) |
| Generate | `scripts/gpu/wan_gen.py {t2v\|i2v\|s2v}` — **`s2v` = audio-driven avatar, the HeyGen replacement (VERIFIED: lip-sync confirmed 2026-09-17)**; **`i2v` = 14B two-expert MoE from an art-directed first frame, the quality tier** (built + tested, not yet rendered); `t2v` = drafts only, reads dated. Measured: t2v 832x480/49f/20steps = 82s; s2v fp8 ~11 s/frame. Graphs pinned by `scripts/tests/test_wan_gen_graph.py` (28 tests). |
| Known artifact | First ~2 pixel frames of every `s2v` take are soft/over-warm (temporally-causal VAE, no context for the opening latent). **Fix by SUBSTITUTION after render, never by dropping frames** — dropping shifts ~125ms against the audio at 16fps and lip-sync breaks past ~50ms. Source-level `--frame0-fix` exists but is opt-in and UNVERIFIED (may change output frame count). |
| Shared queue | **One GPU, one ComfyUI queue, two agents (Bravo + Maven).** Claim the box before submitting — concurrent jobs serialise at best, thrash VRAM at worst. |
| **Cost model** | **75 credits/hour whenever the instance EXISTS.** Cannot be paused — only Reboot or Destroy. Credits hitting zero destroys the box *with its data*. `/opt/oasis/outputs` is the only irreplaceable path — `pull outputs` after every session. |

**Three traps, each verified the hard way (2026-09-17):** (1) the instance is a *container* — installing Docker pulls `apparmor`, which cannot configure on its overlayfs and leaves `apt` permanently broken; Docker is skipped by design. (2) The provider ships `/etc/ssh/sshd_config.d/99-haishare.conf` with `PasswordAuthentication yes`; sshd takes the *first* value lexically, so a hardening drop-in must sort **before** it (`00-`). (3) Official `Wan-AI/*` weights are sharded diffusers and **ComfyUI cannot load them** — use `Comfy-Org/Wan_2.2_ComfyUI_Repackaged`.

Maven's operating handover: `~/CMO-Agent/HANDOVER_MAVENGPU_VIDEO_GENERATION.md`.
...
- **ScrapeGraphAI** — primary public-site and lead-data scraper: markdown, prompt/schema extraction, search, async crawl, and credit checks. `scripts/integrations/scrapegraph_tool.py`.
- **Firecrawl** — public-provider fallback and one-shot site mapping. `scripts/integrations/firecrawl_tool.py`.
...
| **research_fetch** *(DEFAULT)* | `scripts/research_fetch.py` | ScrapeGraphAI primary → Firecrawl fallback → Cloak anti-bot escalation → urllib, with SQLite domain reputation. | `fetch <url>`, `reputation`, `reputation-clear`, `--force-tier {scrapegraph,firecrawl,cloak,plain}` |
| **ScrapeGraphAI** *(2026-08-31, PRIMARY)* | `scripts/integrations/scrapegraph_tool.py` | Public scraping, structured lead extraction, search, crawl, and credits. | `scrape`, `extract`, `search`, `crawl-start`, `crawl-status`, `credits` |
...
| **Canonical env store** | `.env.agents` + `scripts/lib/env_store.py` | One plaintext store in this repo. Locked read-modify-write plus same-directory atomic replacement prevents concurrent saves, partial files, and backup fan-out. Encrypted off-box recovery is the only supported backup form. |
...
| **Cloudflare Workers (PRIMARY deploy engine)** | `scripts/integrations/wrangler_tool.py` | The fleet's deployment, secret-push, and log-tail engine (2026-08-29 Vercel→Workers migration). Registry: `config/cloudflare/apps.json`; secret manifests (key NAMES only): `config/cloudflare/manifests/`. Verbs: `whoami`/`accounts` (run FIRST — key presence ≠ right account) · `zones`/`list-workers`/`subdomain`/`registrar-status` · `secrets-plan` (gap diff vs env store) · `secrets-push` (values via stdin, never argv) · `secrets-list` (names only) · `upload` (remote preview version, no traffic) · `scaffold` (stamps wrangler.jsonc + open-next.config.ts) · `workflow` (generates the repo's `.github/workflows/deploy-cloudflare.yml` from registry + manifest) · `zone-baseline` (snapshots every zone's DNS to `state/cloudflare_baselines/`; shouts if an ACTIVE zone has no MX) · **`attach-domain --app X --hostname H`** (the ONLY sanctioned way to point a hostname at a Worker — prints rollback values, and ABORTS when the live host and the Worker serve different pages or the Worker is undeployed; `--dry-run`/`--force`) · `build`/`preview`/`deploy` (deploy-first ordering; full env at build) · `tail --seconds N` (bounded sampler, tree-kill). Secrets flow store→child env only — the LLM never sees values. |
...
| Cloudflare Workers | GitHub Actions auto-deploy | Primary hosting and previews |
...
_Last synced: 2026-09-22T08:02:46.279910+00:00_

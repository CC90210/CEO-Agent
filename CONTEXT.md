...
- **Founder-led sales (2026-09-24)** — The OASIS sales team is retired; CC and Adon run all sales and marketing. The only reps are **David (opener)** and **Schneur (builder)**, the only people with commission logic. Canonical: `APPS_CONTEXT/OASIS_AI_CLAUDE.md` § Sales Roles.
- **Active / Inactive teammate** — A deactivated command-center profile (`user_profiles.deactivated_at`, migration 179): hidden from every live roster (assign lists, rep chips, scorecard, commissions, form routing), login banned + sessions killed, history kept, reversible from Settings → Team. Never "delete a rep" — deactivate. Code: `lib/team-activation.ts`.
- **Revenue cycle** — The reversible view boundary on the OASIS pipeline board (`lib/pipeline-cycle.ts`, current id `revenue-2026-09-23`). The board shows only leads assigned/stamped in the cycle; older leads stay in `tenant_records`, untouched. "The pipeline is empty" after a reset means the cycle boundary, not deleted data.
- **Revenue goal / October sprint** — A `revenue_goals` row (migration 182) with a target and a period; Today computes progress from the Finances ledger. Current: **≥ US$6,000 collected, 2026-09-24 → 2026-10-24**. Collected = money received net of refunds, converted at each payment's own-day Bank of Canada rate. Atlas owns the number. Supersedes the retired "$10K Net MRR by 2026-09-30".
- **Finances suite** — Command Center **FOUNDERS → Finances** (founders only): double-entry books for the OASIS business entity and each founder's private personal entity, invoicing from the OASIS mailbox with Stripe pay links, bank CSV/OFX import + rules, reports, GST/QST tracking (OASIS is a small supplier — not registered). Modelled on bigcapital's data model, no AGPL code copied. Atlas runs its automations via `/api/internal/finance/*`.
...
- **North Star (Bravo)** — Multiply CC's time and ship the systems that scale the empire. Bravo does **not** optimize for a dollar metric. All revenue targets, MRR, and deal economics are owned by **Atlas (CFO-Agent)** — route money questions there. The current company revenue goal is the **Revenue goal / October sprint** entry above; the old "$10K Net MRR by 2026-09-30" target is retired (2026-09-24).

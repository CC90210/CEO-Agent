...
## Spillover (Claude usage-limit failover, 2026-09-12)

Terms for keeping Claude Code working through a Claude Max usage limit. Contract:
[[scripts/spillover/CONTRACT]]. Decision: [[docs/adr/0018-claude-spillover]]. Operator skill:
[[skills/claude-spillover/SKILL]]. **Spillover is neither the messaging gateway** (`gateway/`, the
Telegram / Discord / Slack adapter layer) **nor `send_gateway.py`** (the outbound-communication chokepoint):
it routes model traffic only and sends nothing on anyone's behalf. Never call any part of it "the gateway".

- **Claude Spillover** — The system that keeps an interactive Claude Code session (IDE panel or CLI) working when the Claude Max subscription hits its account-wide usage limit: the same conversation continues on OmniRoute's `bravo-fallback` combo until the limit resets, then returns to Claude, with no manual step. Deployed outside the repo to `%LOCALAPPDATA%\bravo-spillover\` (SHA-stamped) and supervised by its own `supervisor.js`, not `fleet_watchdog`. Status: `python scripts/integrations/omniroute_tool.py spillover status`.
- **Spillover proxy** — `spillover_proxy.js`, the zero-dependency Node pass-through on `127.0.0.1:20131` that Claude Code reaches through `ANTHROPIC_BASE_URL` with **no credential set** (base-URL-only mode), so the Max login stays the active credential. It forwards every byte unchanged to `https://api.anthropic.com` and never stores or logs the credential. Its `proxy.mode` is `passthrough` (the kill switch: forward everything, never reroute), `observe` (forward everything, record detected limits and header telemetry) or `spill` (full behaviour). Stopping it is never the kill switch — every open session is pointed at it.
- **Fallback leg** — The path a spilled request takes, to OmniRoute on `127.0.0.1:20128`: normalized for OpenAI (server tools, deferred tools, `context_management`, the Claude attribution block and `x-claude-code-*` / `x-stainless-*` headers stripped), authenticated with the OmniRoute lane key and never with a `sk-ant-*` value (asserted in code and tested), routed with `X-Route-Model: bravo-fallback`, and streamed back with thinking blocks dropped so transcripts never carry unsigned thinking. Its opposite is the **direct leg**: byte-for-byte to Anthropic, about 99% of traffic.
- **Subscription usage-limit 429** — The one Anthropic reply that triggers a spill: status 429 **and** header `anthropic-ratelimit-unified-status: rejected` (the `…-overage-status` header never counts) **and** `error.type == rate_limit_error` **and** representative claim `five_hour` or `seven_day` **and** `unified-fallback` not `available` **and** overage not in use. A model-scoped limit (`seven_day_opus`, `seven_day_sonnet`, … — kind `model_limit`) and every other 429 or 529 (kind `transient`) are **not** spill triggers and pass through untouched. Classified by `limit_detector.js` `classify()`; the reset time comes from `anthropic-ratelimit-unified-reset`, skew-corrected against the `Date` header and clamped between now + 60 s and now + 8 days.
- **Spill eligibility** — The per-request test for whether a request *may* take the fallback leg. All must hold: `proxy.mode` is `spill`; `POST /v1/messages`; an OAuth bearer (`sk-ant-oat…`, a claude.ai login — any request carrying `x-api-key` is never eligible); a user-agent entrypoint in `proxy.spill_entrypoints` (`cli`, `claude-vscode`); and no `X-Bravo-Lane: automation` or `X-Bravo-Spill: deny` header. An ineligible request during a spill goes direct and its 429 passes through. Eligibility decides *who* may spill; the subscription usage-limit 429 decides *when*.
- **Automation pin** — `config/spillover/automation_pin.json` (`{"env":{"ANTHROPIC_BASE_URL":"https://api.anthropic.com"}}`), passed as `--settings <abs path>` by `run_claude_cli` and `run_claude_cli_on_document` in `scripts/lib/claude_cli.py`, so daemon automations reach Anthropic directly even while user settings point at the proxy. Second guard: the spawn env adds `ANTHROPIC_CUSTOM_HEADERS=X-Bravo-Lane: automation`, after `build_claude_spawn_env` strips the inherited `lane_env_strip` vars. Automations therefore never spill; client-facing ones hold for review when anything other than Claude answers (ADR-0018 Decision 5).
- **Per-repo spill deny** — A repo opts out of spilling by setting `ANTHROPIC_CUSTOM_HEADERS=X-Bravo-Spill: deny` in the env block of its `.claude/settings.local.json`. SunBiz-Agent carries it by default because it holds merchant PII. Its requests always go direct; at a limit they get Anthropic's 429, exactly as before Spillover existed.
- **`bravo-fallback`** — The OmniRoute combo the fallback leg routes to. A *combo* is OmniRoute's term for an ordered model list it tries in turn: `cx/gpt-5.6-sol`, `cx/gpt-6-astra`, then Cerebras and Cloudflare Workers AI (`cf/`) models that passed the tool-use smoke gate (`omniroute_tool.py smoke`). `bravo-fallback-fast` is its sibling for Haiku-class requests. The lane key is scoped to these two combos only, and no forbidden prefix (`cc/`, `oc/`, `kiro/`, `agy/`, `gemini/`, `groq/`, `glm-cn/`, …) may appear in either.

...
- **`bravo-fallback`** — The OmniRoute combo the fallback leg routes to. A *combo* is OmniRoute's term for an ordered model list it tries in turn: `cx/gpt-5.6-sol`, `cx/gpt-6-astra`, then Cerebras / Z.AI models that passed the tool-use smoke gate (`omniroute_tool.py smoke`). `bravo-fallback-fast` is its sibling for Haiku-class requests. The lane key is scoped to these two combos only, and no forbidden prefix (`cc/`, `oc/`, `kiro/`, `agy/`, `gemini/`, `groq/`, `glm-cn/`, …) may appear in either.

...
- Cedarwood Wellness + Garden Holistics → log no-answer calls
...
# ============================================================
cw = db.table("leads").select("id,notes").eq("company", "Cedarwood").execute().data or []
for l in cw:
    ...
    print("[call logged] Cedarwood Wellness — no answer")
...
for l in skillet:
    fixed_notes = (l.get("notes") or "").strip() + (
        ...
        "email on file (theironskilletwasaga@gmail.com) belongs to the WASAGA "
    ...
    db.table("lead_interactions").insert({
        ...
        "content": (
            "Email on file (theironskilletwasaga@gmail.com) belongs to the "

---
name: Monthly Investor Update
type: document
use_case: Monthly email update to investors, advisors, or stakeholders
variables: [month, year, headline, mrr, arr_growth, clients, pipeline_value, biggest_win, biggest_challenge, asks]
tags: [template, document, investor]
---

# {{month}} {{year}} Update — OASIS AI Solutions

**From:** Conaugh McKenna, Founder
**Subject:** OASIS — {{month}} Update: {{headline}}

---

## TL;DR
- **Biggest win:** {{biggest_win}}
- **Key metric:** {{mrr}} MRR ({{arr_growth}}% MoM growth)
- **Biggest challenge:** {{biggest_challenge}}

---

## Key Metrics

| Metric | This Month | Last Month | Change |
|--------|-----------|-----------|--------|
| MRR | ${{mrr}} | $ | +/-X% |
| Active Clients | {{clients}} | | +/-X |
| Pipeline Value | ${{pipeline_value}} | $ | |
| Content Published | X posts | | |
| Inbound Leads | X | | |

---

## What Shipped

- [Feature/deliverable 1]
- [Feature/deliverable 2]
- [Client win — specific result]

## What We Learned

- [Insight 1 — what surprised us]
- [Insight 2 — what we're changing]

## Challenges

- [Challenge 1 — what's hard right now]
- [Challenge 2 — how we're addressing it]

## Team

- [Any changes — hires, departures, capacity]
- Current team size: [X]

## Next Month Focus

1. [Priority 1]
2. [Priority 2]
3. [Priority 3]

## Asks

{{asks}}

[Specific things you need from investors/advisors — intros, advice, resources]

---

Best,
Conaugh McKenna
Founder, OASIS AI Solutions

*Only good things from now on.*

## Obsidian Links
- [[data/INDEX]] | [[brain/CEO_OPERATING_SYSTEM]]

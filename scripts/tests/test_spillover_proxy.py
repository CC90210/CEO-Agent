...
def rig(tmp_path,anthropic,omni,mode="spill",extra=None,now_offset=0,initial_state=None):
    procs=[]
...
def test_security_gates_and_poison(tmp_path):
    ...
    with rig(tmp_path/"poison",anth,omni) as (port,state,*_):
        body=b'{"model":"claude-sonnet","messages":[{"role":"user","content":"sk-ant-poison"}]}'
        assert request(port,body=body)[0]==429
        events=(state/"events.jsonl").read_text();assert OAUTH not in events and "sk-ant-poison" not in events

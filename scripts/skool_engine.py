...
def _extract_comments_on_post(page) -> list:
    ...
    Returns a list of dicts: [{idx, author, content, is_cc, is_bennett}, ...]
    ...
    try:
        comments = page.evaluate(r"""() => {
            ...
            for (const w of wrappers) {
                ...
                const isBennett = (
                    authorTrimmed === 'bennett'
                    || authorTrimmed === 'bennett spooner'
                    || authorTrimmed.startsWith('bennett ')
                );
...
def generate_comment_reply(
    ...
    system = (
        ...
        "OASIS AI Solutions and head coach of the Agency Accelerants community. The community is "
...
def _process_post_comments(
    ...
    for c in comments:
        ...
        # Never reply to CC, Bennett, or empty comments
        if c.get("is_cc") or c.get("is_bennett"):
            continue

...
def _run_harness(args: list[str], timeout: int = 60) -> dict[str, Any]:
    """Run the browser harness CLI script."""
    cli = Path(__file__).resolve().parent / "browser_harness_cli.py"
    if not cli.exists():
        return {"ok": False, "stdout": "", "stderr": "browser_harness_cli.py not found", "returncode": 1}
    return _run([sys.executable, str(cli), *args], timeout=timeout)
...
def diagnose() -> dict[str, Any]:
    ...
    checks.append({
        ...
        "ok": cli.exists(),
        "required": True,
        "detail": "present" if cli.exists() else "missing",
    })

    version = _run_harness(["--version"], timeout=30)
    doctor = _run_harness(["--doctor"], timeout=60)

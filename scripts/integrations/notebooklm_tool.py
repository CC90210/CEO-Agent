...
Usage (from any agent via terminal):
  python scripts/integrations/notebooklm_tool.py list
  python scripts/integrations/notebooklm_tool.py create --title "Research Topic"
  python scripts/integrations/notebooklm_tool.py use <notebook_id>
  python scripts/integrations/notebooklm_tool.py status
  python scripts/integrations/notebooklm_tool.py ask "What are the key findings?"
  python scripts/integrations/notebooklm_tool.py source add <url_or_file>
  python scripts/integrations/notebooklm_tool.py source list
  python scripts/integrations/notebooklm_tool.py generate audio [--wait]
  python scripts/integrations/notebooklm_tool.py generate report [--wait]
  python scripts/integrations/notebooklm_tool.py download audio <output_path>
  python scripts/integrations/notebooklm_tool.py summary
...
def main():
    ...
    args = sys.argv[1:]
    if not args:
        print("Usage: python scripts/integrations/notebooklm_tool.py <command> [args...]")

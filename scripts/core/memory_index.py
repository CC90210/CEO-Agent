...
Usage:
  python scripts/core/memory_index.py build          Build/rebuild the index from all memory files
  python scripts/core/memory_index.py search <query> Search the index for relevant topics
  python scripts/core/memory_index.py stats          Show index statistics
  python scripts/core/memory_index.py --json         Machine-readable output
"""
...
def show_stats(output_json=False):
    ...
    if output_json:
        ...
        print(f"  Index:        {'exists' if stats['index_exists'] else 'NOT BUILT -- run: python scripts/core/memory_index.py build'}")
...
if __name__ == "__main__":
    ...
    if args.command == "build":
        ...
        if not query:
            print("Usage: python scripts/core/memory_index.py search <query>")
            sys.exit(1)

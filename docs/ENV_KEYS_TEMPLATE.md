...
## oasis-store (subscription storefront — its OWN Stripe account)

| Variable | Notes |
|----------|-------|
| `OASIS_STORE_TURSO_DATABASE_URL` / `OASIS_STORE_TURSO_AUTH_TOKEN` | Written by `turso_admin.py create --db oasis-store --write-env` |
| `OASIS_STORE__APP_URL` | Public origin (workers.dev until the domain is bound) |
| `OASIS_STORE__SESSION_SECRET` / `OASIS_STORE__WORKER_SHARED_SECRET` | Written by `oasis_store_stripe_setup.py gen-secrets` |
| `OASIS_STORE_STRIPE_SECRET_KEY` | The store's dedicated Stripe account — `oasis_store_stripe_setup.py whoami` refuses OASIS/PropFlow/Nostalgic ids |
| `OASIS_STORE_STRIPE_WEBHOOK_SECRET` | Written by `oasis_store_stripe_setup.py webhook`, never printed |
| Daemon-only (VPS): `OASIS_STORE_MAIL_USER`, `OASIS_STORE_MAIL_APP_PASSWORD`, `TELEGRAM_CHAT_ID`, `CJ_API_EMAIL`, `CJ_API_KEY`, `OASIS_STORE_LLM`, `OPS_MODE` | See `APPS/oasis-store/docs/ENV_KEYS.md` |

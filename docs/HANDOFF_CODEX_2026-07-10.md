...
1. ~~task_outcomes loop is open~~ **DONE 2026-07-10 (Bravo, executing this handoff):** validator agent now self-records verdicts (validator.md §6, its one permitted write); `scripts/core/codex_review.py` wraps codex-companion and records Codex verdicts (verbatim output preserved per RULE 8); `.github/workflows/substrate-eval.yml` = eval-in-CI on the structural subset (parity + genome_sync --check + agent_genome). First verdict recorded; dashboard live via `task_outcomes.py rate`. AUDIT ANGLE FOR CODEX: verify the validator actually records in practice (its next few runs), and consider a weekly rate-trend line in the daily brief.

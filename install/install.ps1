...
if (-not $SkipSmokeTests) {
    Step "Running self_audit + browser_harness_doctor" {
        & python "$repoRoot\scripts\core\self_audit.py" 2>&1 | Select-Object -Last 5 | Write-Host
        if ($LASTEXITCODE -ne 0) {
            Write-Host "    WARN: self_audit exited with code $LASTEXITCODE" -ForegroundColor Yellow
        }
        if (Test-Path "$repoRoot\scripts\browser\browser_harness_doctor.py") {
            & python "$repoRoot\scripts\browser\browser_harness_doctor.py" --json 2>&1 |

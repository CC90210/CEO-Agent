...
>
> ⚠ **Read the freshness gate first.** If `last_updated` above is older than 7 days, treat every claim below as archived. Re-verify with `python scripts/core/self_audit.py --json`, `git log --since="7 days ago"`, and direct tool calls (`revenue_engine.py mrr`, `lead_engine.py pipeline`) before quoting anything as truth.
>
...
| **Context Builder** | ✅ V5.6 LIVE | `scripts/core/context_builder.py` — relationship stage + sentiment + prompt composition. Feeds persona-aware drafts. |
...
| **Google Workspace CLI** | ✅ FULLY CONNECTED | `scripts/integrations/google_tool.py` wraps gws v0.18.1 + SMTP fallback. oasisaisolutions@gmail.com authenticated. 14 OAuth scopes. 5 integration tests passing. |
...
| **Browser Harness** | ✅ LIVE | Daemon attached to Chrome 147 via CDP port 9222. Skills: `skills/browser-harness/SKILL.md`, `browser/` dir. Doctor: `scripts/browser/browser_harness_doctor.py`. Auto-start shortcut in Startup folder. |
...
| **Semantic Memory** | ✅ LIVE | `scripts/integrations/mem0_tool.py` — Qdrant embedded, fastembed, Claude Haiku extraction. |
...
| **Firecrawl** | ✅ LIVE | `scripts/integrations/firecrawl_tool.py` + MCP server. Web scraping and structured extraction. |
...
| Memory fragmentation (5 systems) | MEDIUM | `scripts/state/state_sync.py` — single-write protocol deploying. |
...
Run `python scripts/state/state_sync.py --note "<one-line summary>"` after edits. The 7-day freshness gate is enforced by `memory_aging.py` — drift past it and the agent will treat this content as archived.

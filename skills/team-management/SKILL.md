...
This skill gives CC a repeatable system to hire, onboard, manage, and eventually offboard the right people — contractors first, full-time later. Every template is designed to be run quickly (CC's time is scarce) and to extract high signal at every stage.
...
Compensation range: ($X/hr or $X/month — never negotiate from zero)
...
Required skills (deal-breakers — must have ALL of these):
-
-

Nice-to-haves (bonus signals, not filters):
-
-

NOT this role (what it explicitly isn't — prevents misaligned applicants):
-
-

Success in 30 days looks like:
...
```
[HOOK — 1 sentence about why this role matters at this company]
[WHO WE ARE — 2-3 sentences about OASIS/PropFlow/the brand]
[THE ROLE — 3-4 sentences on what this person actually does day-to-day]
[WHAT YOU'LL DO — 4-6 bullets from the core responsibilities]
[WHAT WE NEED — 3-5 required skills as bullets]
[WHAT'S IN IT FOR YOU — compensation, schedule, remote, growth opportunity]
[HOW TO APPLY — specific instruction that filters lazy applicants
...
**Minimum hiring bar:** Weighted average ≥ 3.5. Never hire below 3.0.
...
- Can't give specific examples — everything is "we did X" (not "I did X")
- Blames former employers in detail — signals pattern, not isolated incident
- Hasn't researched the company at all — signals low interest or high entitlement
- Negotiates aggressively before seeing the offer — signals misaligned priorities
- Can't explain why they left their last role clearly — often hiding performance issues
- Answers questions with questions to buy time repeatedly — signals gaps they're masking
...
- Pushes back on a weak assumption you make — thoughtfully and respectfully
...
1. "On a scale of 1-10, how strongly would you rehire [Name] if you had an open role they were suited for? What would need to be true for that number to be higher?"
   (Forces honesty — anything below 8 is a red flag. The "what would need to be true" part surfaces real weaknesses.)

2. "What type of manager brings out the best in [Name]? And what type of working environment is NOT a good fit?"
   (Reveals how much management the person needs — critical for a lean team where CC has limited bandwidth.)
...
- [ ] NDA signed via DocuSign or equivalent (use `scripts/contract_generator/generator.py` if OASIS)
- [ ] Contractor agreement signed (scope, rate, payment terms, IP assignment)
- [ ] Payment method confirmed (Wise, PayPal, Stripe, e-transfer)
- [ ] Payment schedule confirmed (weekly, bi-weekly, or milestone-based)
- [ ] Invoice format agreed (they submit → CC approves within 48h → paid within 72h)

### Day 1: Access Provisioning
- [ ] Email added to relevant Google Workspace or given CC's shared credentials via 1Password
- [ ] GitHub repo access granted (minimum permissions required)
- [ ] Slack/Discord added to relevant channels (not every channel)
- [ ] Project management tool access (Notion, Linear, or Asana if applicable)
- [ ] Loom account access or record a 10-min welcome walkthrough (saves 3 async Q&A threads)

### Day 1: Context Download
- [ ] Brand guide sent (CC's voice, values, visual identity)
- [ ] Relevant SOPs sent (only what applies to their role — don't overwhelm)
- [ ] Key contacts introduced (CC, Adon if PropFlow, any current clients they'll interact with)
- [ ] Stated outcome for their first 30 days in writing

### Day 3: First Task Assignment
- [ ] Task has a clear title, deliverable, success criteria, and deadline
- [ ] Resources needed are provided upfront (no scavenger hunts)
- [ ] Preferred communication channel confirmed (Slack DM vs. email for each type of question)
- [ ] Explicitly stated: "Ask before assuming. Wrong direction for 2 hours costs more than a 5-min question."

### Day 7: First Deliverable Check-In
- [ ] Review the first deliverable against the success criteria (not subjective feel)
- [ ] Give specific feedback (not "good job" — "section 2 needs X because Y")
- [ ] Adjust scope, timeline, or communication frequency if needed
- [ ] Ask: "Is there anything about how we're working together that's slowing you down?"
...
### Day 30: Full Review
Formal conversation:
1. Review deliverables produced vs. committed deliverables
2. Performance snapshot scores across the month
3. Feedback exchange: CC gives feedback, then asks "What's one thing I could do better as a client?"
4. Decision: Continue at same terms / Adjust scope or rate / End engagement
5. If continuing: set next 30-day goals explicitly
...
```
WEEKLY 1:1 — [Name] — [Date]
─────────────────────────────
1. How are you doing? (Genuine check-in — not just work)
   Notes:
...
```
QUARTERLY REVIEW — [Name] — [Quarter: Q1/Q2/Q3/Q4 YYYY]
──────────────────────────────────────────────────────────
Role:
Review period:

GOALS ACHIEVED
Goal 1: [stated goal] → [outcome with evidence]
Goal 2: [stated goal] → [outcome with evidence]
Goal 3: [stated goal] → [outcome with evidence]
Overall goal achievement: X/3 goals fully met

AREAS OF STRENGTH (be specific — name actual deliverables)
-
-

AREAS FOR IMPROVEMENT (specific and actionable — not vague)
Improvement area:
Why it matters:
...
Performance rating: [1-5 — see scale below]
...
**Rating Scale:**
- 5 — Exceeds: consistently delivers beyond scope, proactively improves processes
- 4 — Meets+: meets all goals and frequently goes beyond on quality or speed
- 3 — Meets: delivers what was agreed, rarely more, rarely less
- 2 — Below: missing goals or quality issues requiring repeated correction
- 1 — Critical: performance gap threatening project outcomes; PIP required or end engagement
...
- **A — Accountable:** Owns the outcome; the one CC asks "did it happen?"
- **C — Consulted:** Input needed before decision is finalized
- **I — Informed:** Kept in the loop but not required for decision
...
WHAT: (specific deliverable — what does done look like in 1 sentence?)

WHY: (why this task matters — context that changes how they approach it)
...
```
CAPACITY SNAPSHOT — [Week of DATE]
────────────────────────────────────
Name          | Role        | Hrs/week | Projects          | Utilization
...
| Any role | >90% for 4+ consecutive weeks | Hire signal — add headcount or reduce scope |

### When to Hire Next

Hire indicator is met when ANY of these are true for 4+ consecutive weeks:
1. Your own utilization (CC's hours on delegatable work) is >90%
2. A contractor's utilization is >90% and the work is still growing
3. A critical skill is routinely absent (e.g., no one owns design and it's blocking launches)
4. Client onboarding speed is limited by team capacity, not sales pipeline
...
```
STATUS — [Name] — [Date]
✅ DONE: [What was completed this week — 2-4 items]
🔄 DOING: [What's in progress right now]
🚫 BLOCKED: [Anything that's stuck and what it's waiting on]
...
- [ ] All work-in-progress documented (status, next steps, where files live)
- [ ] All credentials they created or modified updated in 1Password / `.env.agents`
- [ ] All ongoing automations documented (n8n, cron jobs, scheduled tasks they owned)
- [ ] Client-facing work handed off to CC or next person with context note
- [ ] All project files organized in the agreed folder structure
- [ ] Loom walkthrough recorded for complex processes they owned

### Access Revocation Checklist

Revoke on or before last day:
- [ ] Remove from GitHub org / repos
- [ ] Revoke Supabase access (check all projects they had access to)
- [ ] Remove from Slack workspace
- [ ] Remove from any project management tool
- [ ] Change shared passwords they had access to
- [ ] Revoke any API keys issued specifically to them
- [ ] Remove from email lists or forwarding rules

### Final Deliverables Handoff

```
OFFBOARDING HANDOFF — [Name] — [Date]
──────────────────────────────────────
Projects handed off:
...
1. "What was the most valuable thing you worked on here?"
2. "What was the most frustrating part of working with us? What could we do better?"
3. "Is there anyone in your network who might be a great fit for the work you did?"

---

## Integration Points

- **Hiring decisions** → log to `memory/DECISIONS.md`
- **Performance issues** → log to `memory/MISTAKES.md` with root cause and prevention
...
- [[skills/ceo-dashboard/SKILL.md]] | [[skills/project-management/SKILL.md]]
- `/onboard-team-member`

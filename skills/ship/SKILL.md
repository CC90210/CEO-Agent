...
## Phase 8.5: Which host is this app on? (added 2026-08-30)

The fleet is mid-migration from Vercel to **Cloudflare Workers**. Check
`config/cloudflare/apps.json` before assuming the host:

- **Listed there** → Cloudflare Workers is the target. The deploy engine is
  `python scripts/integrations/wrangler_tool.py` — the primary deploy,
  secret-push and log-tail path for every app:
  ```
  wrangler_tool.py whoami                      # FIRST — a present key ≠ the right account
  wrangler_tool.py secrets-plan --app <slug>   # gap-check before deploying
  wrangler_tool.py deploy     --app <slug>     # build → deploy → secrets push
  wrangler_tool.py tail       --app <slug> --seconds 120
  ```
  Each of those repos also carries `.github/workflows/deploy-cloudflare.yml`
  (push to `main` → build → `wrangler-action@v3`), generated by
  `wrangler_tool.py workflow --app <slug>`.
- **Not listed** (e.g. listing-studio, dropped 2026-08-29) → still Vercel;
  use the Vercel steps below.
- **Either way, never hand-edit secrets into a repo.** Values live in the
  agents env store; `secrets-push` injects them into the child process only.

Cutover status + per-app gates: `brain/DNS_CUTOVER_AND_VERCEL_EXIT_CHECKLIST.md`.

## Phase 9: Post-Ship Verification

After the host deploys (Vercel: usually 1-3 minutes after push; Cloudflare:
`wrangler_tool.py deploy` reports the version id synchronously):

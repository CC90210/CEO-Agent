...
|---|---|---|---|
| 1 | **Secret files committed to git** | `python scripts/scan_secrets.py --history` (a working-tree scan misses leaked-then-deleted); `git ls-files \| grep -Ei 'env\|token\|credential\|\.pem\|id_rsa'` | Credentials live in the agents env file only — gitignored, and unreadable by the model (`secret_guard`). A tracked secret is rotated at the provider **first**, scrubbed from history second. |

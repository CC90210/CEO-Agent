...
|---|---|---|---|
| 1 | Secrets reachable through the repo — committed, *or* in `.git/config` | `git ls-files \| grep -Ei 'env\|token\|credential\|\.pem\|id_rsa\|service_account'`; scan history too, since a secret deleted in a later commit is still public; **and `git remote -v`** — a `https://<token>@host/...` remote is a live credential in plaintext that no file scan can see, because that file is never committed | No tracked secrets, no credential in any remote URL, `.gitignore` covering env and key patterns. Any hit is rotated at the provider **before** anything is scrubbed. Report the finding WITHOUT echoing the value — name the remote and give the clean URL |

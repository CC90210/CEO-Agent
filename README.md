...
| **V6 Apex** | 2026-05-10 | Cross-agent event bus (Postgres `agent_events` with LISTEN/NOTIFY + `claim_events` SKIP LOCKED), hybrid semantic memory (FTS5 + LanceDB RRF), dashboard-driven override approvals, `/feed` view | `brain/EVENT_BUS_CONTRACT.md`, `scripts/{event_router,exec_override_consumer}.py`, [`app/(internal)/feed/page.tsx`](https://github.com/CC90210/oasis-command-center/blob/main/app/feed/page.tsx) |

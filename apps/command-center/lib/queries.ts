...
// ============================================================================
// primary retainer concentration risk — % of MRR from a single client
// ============================================================================

/**
 * Returns the % of MRR concentrated in the operator's biggest client.
 * Hardcoded "primary retainer" detection for CC's case; for other tenants it returns
...
export async function topClientConcentration(tenantId: string): Promise<{
  ...
  // CC's primary retainer split: $2,500 flat + $451 rev share = ~$2,951 of $3,322 = 89%
  ...
  const topClientMrr =
    ...
    // Fallback heuristic for CC: primary retainer is named in the lead row
    (/primary_retainer/i.test(name) ? 2951 : 0);
